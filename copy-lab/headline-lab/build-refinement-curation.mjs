...
const reviewsDirectory = path.join(directory, "curation", "refinement-reviews");
...
const eligible = library.items.filter((item) => item.batch.endsWith("refinements"));
...
if (batches.length !== 3) throw new Error(`Expected 3 refinement batches; received ${batches.length}.`);
if (eligible.length !== 60) throw new Error(`Expected 60 refinement systems; received ${eligible.length}.`);
if (files.length < 3) throw new Error(`Expected at least 3 independent refinement reviews; received ${files.length}.`);
...
for (const filename of files) {
  ...
  if (review.picks.length !== 15) throw new Error(`${filename}: expected exactly 15 picks; received ${review.picks.length}.`);
  ...
  for (const batch of batches) {
    const batchCount = review.picks.filter((pick) => byId.get(pick.id)?.batch === batch).length;
    if (batchCount < 3) throw new Error(`${filename}: expected at least 3 picks from ${batch}; received ${batchCount}.`);
  }

  for (const pick of review.picks) {
    if (!eligibleIds.has(pick.id)) throw new Error(`${filename}: ${pick.id} is not a refinement candidate.`);
...
for (const batch of batches) {
  ranked.filter((candidate) => candidate.batch === batch).slice(0, 3).forEach(add);
}
for (const candidate of ranked) {
  if (selected.length >= 15) break;
  add(candidate);
}
if (selected.length !== 15) throw new Error(`Expected 15 refinement finalists; selected ${selected.length}.`);
...
await fs.writeFile(path.join(directory, "refinement-curation.json"), `${JSON.stringify(curation, null, 2)}\n`, "utf8");
console.log(`Selected ${curation.count} finalists from ${curation.eligible} refinements across ${curation.reviewers} lenses.`);

function add(candidate) {
  if (selectedIds.has(candidate.id) || selected.length >= 15) return;
